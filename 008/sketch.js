...
let buttonX = 1100, buttonY = 615, buttonScale = 0.25;  // 定义按钮位置和缩放比例
...
function preload() {
  ...
  bgImage = loadImage('未标题-1-13.jpg');  // 替换为你的背景图片路径
  buttonImg = loadImage('未标题-1-14.png');  // 替换为你的按钮图片路径
...
function setup() {
  ...
  particles = new Array(maxParticles).fill().map(() => [
    random(1000, 1120), // x
    random(240, 270), // y
    random(-2, 2), // velocityX
    random(-2, 2), // velocityY
  ]);
// 播放背景音乐并设置音量
if (bgMusic) {
...
if (hoverSound.isPlaying()) {
  ...
  if (isHovered && mouseIsPressed) {
    window.location.href = "../004/index.html";  // 替换为你的跳转链接
  }
